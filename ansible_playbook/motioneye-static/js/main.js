...
function runLoginDialog(retry) {
    ...
    var form =
            $('<form action="' + basePath + 'login/" target="temp" method="POST"><table class="login-dialog">' +
                ...
                '</tr>' +
                '<tr>' +
                    '<td class="dialog-item-label"><span class="dialog-item-label">Потребител</span></td>' +
                    '<td class="dialog-item-value"><input type="text" name="username" class="styled" id="usernameEntry" autofocus></td>' +
                '</tr>' +
                '<tr>' +
                    '<td class="dialog-item-label"><span class="dialog-item-label">Парола</span></td>' +
                ...
                '</tr>' +
                '<tr>' +
                    '<td class="dialog-item-label"><span class="dialog-item-label">Запомни ме</span></td>' +
    ...
    var params = {
        ...
        content: form,
        buttons: [
            {caption: 'Отмени', isCancel: true, click: function () {
                tempFrame.remove();
            }},
            {caption: 'Влез', isDefault: true, click: function () {
...
function runPictureDialog(entries, pos, mediaType) {
    ...
    runModalDialog({
        ...
        buttons: [
            {caption: 'Затвори'},
            {caption: 'Свали', isDefault: true, click: function () {
                var entry = entries[pos];
...
function runAddCameraDialog() {
    if (Object.keys(pushConfigs).length) {
        return runAlertDialog('Първо приложете зададените промени');
    }

    var content =
            $('<table class="add-camera-dialog">' +
                '<tr>' +
                    '<td class="dialog-item-label"><span class="dialog-item-label">Вид камера</span></td>' +
                    '<td class="dialog-item-value"><select class="styled" id="typeSelect">' +
                        (hasLocalCamSupport ? '<option value="v4l2">Локална V4L2 камера</option>' : '') +
                        (hasLocalCamSupport ? '<option value="mmal">Локална MMAL камера</option>' : '') +
                        (hasNetCamSupport ? '<option value="netcam">Мрежова камера</option>' : '') +
                        '<option value="motioneye">Отдалечена motionEye камера</option>' +
                        '<option value="mjpeg">Обикновена MJPEG камера</option>' +
                    '</select></td>' +
                    '<td><span class="help-mark" title="Видът на камерата, която добавяте">?</span></td>' +
                '</tr>' +
                ...
                '<tr class="motioneye netcam mjpeg">' +
                    ...
                    '<td><span class="help-mark" title="потребителското име за този URL, ако е необходимо (напр. admin)">?</span></td>' +
                '</tr>' +
                '<tr class="motioneye netcam mjpeg">' +
                    '<td class="dialog-item-label"><span class="dialog-item-label">Парола</span></td>' +
                    '<td class="dialog-item-value"><input type="password" class="styled" id="passwordEntry" placeholder="password..."></td>' +
                    '<td><span class="help-mark" title="паролата за това потребитлско име, ако са необходими за този URL">?</span></td>' +
                '</tr>' +
                '<tr class="v4l2 motioneye netcam mjpeg mmal">' +
                    ...
                    '<td><span class="help-mark" title="камерата, която добавяте">?</span></td>' +
                '</tr>' +
                '<tr class="v4l2 motioneye netcam mjpeg mmal">' +
    ...
    function updateUi() {
        ...
        if (typeSelect.val() == 'motioneye') {
            ...
            addCameraInfo.html(
                    'Отдалечени motionEye камери са тези, които са инсталирани зад друг motionEye сървър. ' +
                    'Добавянето им ще позволи да бъдат гледани и управлявани от тук.');
        }
